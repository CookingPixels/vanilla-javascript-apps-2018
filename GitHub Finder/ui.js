class UI {
  ...
  showProfile(user) {
    // Make sure it works console.log(user);
  ...
  showAlert(message, className) {
    ...
    // Get parent to append it to the DOM
    const container = document.querySelector('.searchContainer');
    // Get search box
    const search = document.querySelector('.search');
    // Insert the alert, takes 2 parameters, what u wanna insert & before what
  ...
  // Empty the user profile if there's no input text
  clearProfile() {
    this.profile.innerHTML = '';
  }
}
